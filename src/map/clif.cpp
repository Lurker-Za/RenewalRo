...
static int32 clif_getareachar(block_list* bl,va_list ap)
{
	...
	case BL_STALL:
		{
			s_stall_data* st = map_id2st(bl->id);
...
/// @param val2
/// @param val3
void clif_status_change_sub(block_list *bl, int32 id, int32 type, int32 flag, t_tick tick, t_tick tick_total, int32 val1, int32 val2, int32 val3)
{
...
void clif_efst_status_change( block_list& bl, block_list& tbl, enum send_target target, efst_type type, t_tick tick, t_tick tick_total, int32 val1, int32 val2, int32 val3 ){
...
void clif_showstallboard(block_list* bl, uint32 stall_id, const char* message)
{
	...
	p.packetType = HEADER_ZC_STORE_ENTRY;
	p.makerAID = stall_id;
	safestrncpy( p.storeName, message, 80);
	clif_send(&p, sizeof(p), bl, AREA);
		...
		case BL_STALL: {
			...
			switch (bl->type) {
				...
				case BL_ELEM:
					...
					break;
				case BL_STALL:
					memcpy(packet.name, ((TBL_STALL *)bl)->name, NAME_LENGTH);
					break;
			}
...
void clif_parse_PurchaseReq2(int32 fd, map_session_data* sd){
	...
	if(p->UniqueID >= START_STALL_NUM) {
		...
		clif_msg_color( *sd, MSI_SEND_PURCHASES_BY_RODEX, color_table[COLOR_CYAN]);
	} else
...
static void clif_parse_ReqTradeBuyingStore( int32 fd, map_session_data* sd ){
	...
	if(p->storeId >= START_STALL_NUM) {
		...
		clif_msg(*sd, MSI_STORE_ASSISTANT_SEND_SELLS_BY_RODEX);
	} else
		buyingstore_trade( sd, p->AID, p->storeId, p->items, packet_len / sizeof( p->items[0] ) );
}
...
void clif_display_pinfo( map_session_data& sd ){
	...
	// TODO: Whoever wants to take the blame, fix this crappy logic below - and yes I know indentation is wrong, but CANT TOUCH THIS! *sings* [Lemongrass]
		...
		//0:PCRoom
		if (pc_isvip(&sd)) {
			details_bexp[PINFO_BASIC] = battle_config.vip_base_exp_increase * battle_config.base_exp_rate / 100;
		...
		//0:PCRoom
		if (pc_isvip(&sd)) {
			details_drop[PINFO_BASIC] = (battle_config.vip_drop_increase * battle_config.item_rate_common) / 100;
		...
		if (pc_isvip(&sd)) {
			...
			if (details_penalty[PINFO_BASIC] == 100)
				...
				if (details_penalty[PINFO_BASIC] < 100) {
					details_penalty[PINFO_BASIC] = 100 - details_penalty[PINFO_PREMIUM];
					details_penalty[PINFO_BASIC] = 0 - details_penalty[PINFO_PREMIUM];
				}
				else
					details_penalty[PINFO_BASIC] = details_penalty[PINFO_PREMIUM] - 100;
			}
...
bool clif_parse_stylist_buy_sub( map_session_data* sd, _look look, int16 index ){
	...
	switch( look ){
		...
		case LOOK_HEAD_TOP: {
			...
			safestrncpy( msg.send_name, "<MSG>2813</MSG>", NAME_LENGTH );
...
void clif_parse_stylist_buy( int32 fd, map_session_data* sd ){
	...
	struct PACKET_CZ_REQ_STYLE_CHANGE3* p = reinterpret_cast<PACKET_CZ_REQ_STYLE_CHANGE3*>( RFIFOP ( fd , 0 ) );
...
void clif_stall_vending_open(map_session_data *sd){
	...
	int len = MAX_INVENTORY * sizeof( STALL_VENDING_UI_OPEN_sub ) + sizeof( PACKET_ZC_STALL_VENDING_UI_OPEN );
...
void clif_stall_showunit(map_session_data *sd, struct s_stall_data *st){
	...
	p.job = st->vd.look[LOOK_BASE];
	p.xPos = st->x;
	p.yPos = st->y;
	p.sex = st->vd.sex;
	p.head = st->vd.look[LOOK_HAIR];
	p.weapon = st->vd.look[LOOK_WEAPON];
	p.shield = st->vd.look[LOOK_SHIELD];
	p.MidAccessory = st->vd.look[LOOK_HEAD_MID];
	p.TopAccessory = st->vd.look[LOOK_HEAD_TOP];
	p.BottomAccessory = st->vd.look[LOOK_HEAD_BOTTOM];
	p.headpalette = st->vd.look[LOOK_HAIR_COLOR];
#if PACKETVER_MAIN_NUM >= 20180801 || PACKETVER_RE_NUM >= 20180801 || PACKETVER_ZERO_NUM >= 20180808
	p.unknow = (st->vd.look[LOOK_ROBE]) * 65536 + st->vd.look[LOOK_CLOTHES_COLOR];
	p.BodyStyle = st->vd.look[LOOK_BODY2];
#else
...
void clif_stall_ui_close(map_session_data *sd, int type, int reason){
	...
	if(reason != STALLSTORE_POSITION && reason != STALLSTORE_LOCATION){
		sd->state.prevend = 0;
		sd->state.workinprogress = WIP_DISABLE_NONE;
...
void clif_stall_buying_list(map_session_data *sd, s_stall_data *st){
	...
	// Safety check before sending the packet
	if (len <= sizeof(struct PACKET_ZC_STALL_BUYING_LIST_REQUEST)) {
		// Log error or handle the case where no items are actually being sent
		// This could be a simple return, or you might log this as an error for debugging.
		ShowError("clif_stall_buying_list: Attempted to send packet with invalid length.\n");
		return;
	}
