...
static int32 find_next_id(int32 &counter, int32 start_value)
{
    if (counter < start_value)
        counter = start_value;

    int32 start_marker = counter;
    int32 candidate = counter;

    for (;;)
    {
        if (!map_blid_exists(candidate))
        {
            counter = candidate + 1;
            return candidate;
        }
        ...
        ++candidate;
        if (candidate < start_value || candidate < 0)
    ...
    ShowFatalError("find_next_id: All IDs are taken.");
    std::exit(1);
    return -1;
}
...
{
    return find_next_id(stall_id, START_STALL_NUM);
}

static int32 stall_getuid(void)
{
    return find_next_id(stall_uid, START_STALL_UID);
}
...
int8 stall_vending_setup(map_session_data* sd, const char* message, const int16 xPos, const int16 yPos, uint8 *data, int32 count)
{
	...
	s_stall_data *st = (s_stall_data*)aCalloc(1, sizeof(s_stall_data));
	...
	if( Sql_Query( mmysql_handle, "INSERT INTO `%s`(`id`, `uid`, `char_id`, `type`, `class`, `sex`, `map`, `x`, `y`,"
		...
		stalls_table, st->vender_id, st->unique_id, st->owner_id, st->vtype, st->vd.look[LOOK_BASE], st->vd.sex == SEX_FEMALE ? 'F' : 'M', map_getmapdata(sd->m)->name, st->x, st->y,
...
bool stall_isStallOpen(uint32 CID, int16 type){
	...
	auto itStalls = std::find_if(stall_db.begin(), stall_db.end(), [&](s_stall_data *const & itst) {
						return (CID == itst->owner_id && type == itst->vtype);
					});
...
void stall_vending_purchasereq(map_session_data* sd, int32 aid, int32 uid, const uint8* data, int32 count)
{
	int32 i, w;
	double z;
	s_stall_data* st = map_id2st(uid);

	nullpo_retv(sd);
	if( st == NULL )
		return; // invalid shop

	if(!stall_isStallOpen(st->owner_id, st->vtype)){
	...
	pc_payzeny(sd, (int32)z, LOG_TYPE_VENDING, NULL);
...
void stall_vending_save(s_stall_data* st){
	for(int32 i = 0; i < st->vend_num; i++){
...
void stall_buying_save(s_stall_data* st){
	for(int32 i = 0; i < st->vend_num; i++){
...
void stall_vending_getbackitems(s_stall_data* st){
	struct mail_message msg_vendor = {};
...
void stall_buying_getbackzeny(s_stall_data* st){
	struct mail_message msg_buyer = {};
...
void stall_remove(s_stall_data* st){
	...
	switch(st->vtype){
		case 0: {
			...
			if (vsd)
				clif_msg(*vsd, MSI_SEND_REST_ITEMS_BY_RODEX);
			st->vid = 0;
		}
			break;
		case 1: {
			...
			if (bsd)
				clif_msg(*bsd, MSI_SEND_REST_ITEMS_BY_RODEX);
			st->bid = 0;
		}
			break;
	}
...
*/
bool stall_searchall(map_session_data* sd, const struct s_search_store_search* s, const s_stall_data* st, int16 type)
{
	int32 c, slot;
	uint32 cidx;

	if( !st->vtype == type ) // not good type
		return true;
...
TIMER_FUNC(stall_init){
	...
	while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
		...
		char* data;
		st = NULL;
		st = (s_stall_data*)aCalloc(1, sizeof(s_stall_data));
		...
		st->id = st->vender_id;
		Sql_GetData(mmysql_handle, 3, &data, NULL); st->vtype = atoi(data);
		...
		st->m = map_mapname2mapid(esc_mapname);
